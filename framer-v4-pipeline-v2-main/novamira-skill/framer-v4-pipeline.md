...
description: Vollstaendiger Workflow fuer die Konvertierung einer Framer-Seite nach Elementor V4 mit dem framer-v4-pipeline-v2 Repository. Enthaelt alle 18 Schritte, Entscheidungslogik, kritische Invarianten und Fehlerbehandlung. Aktualisiert fuer Novamira AdrianV2 v1.0.0.
version: "1.1"
tags: [framer, elementor, v4, pipeline, mcp, novamira, adrianv2]
---
...
1. NIEMALS batch-build-page fuer Framer-Trees -> elementor-set-content verwenden
2. setup-v4-foundation NIEMALS cachen (GV-IDs + GC-IDs sind session-live, laufen ab)
   export-design-system DARF 5 Minuten gecacht werden (read-only, sicher)
...
> Das ist korrekt — `export-design-system` ist read-only und ändert sich nicht
> während eines Pipeline-Durchlaufs. Nur `setup-v4-foundation` darf nie gecacht
...
**Schritt 13** - Layout-Audit (AdrianV2, PFLICHT):
```
...
Tipp: element_id in novamira-adrianv2/add-flexbox oder add-div-block setzen -> element_ids bleiben stabil fuer Patches.
